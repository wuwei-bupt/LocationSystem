...
/* 			pass: {
 		...
 		submitHandler: function() {
			...
			$.ajax({
				url : 'addDepotInertaerosol',//$("#inputForm").attr("action"),
	...
	<div class="container" >
		...
		<form id="inputForm"    method="post" enctype="multipart/form-data">
			...
			<table class="input tabContent">
				...
				<tr><th><span class="requiredField">*</span> 年 度</th><td><input type="text" name="annual" ></td><th>录入时间：</th><td><input type="text" name="reportdate" class="easyui-datebox"/></td></tr>
				<tr><th> 填报人：</th><td><input type="text" name="reporter" class="text"/></td><th> 联系电话：</th><td><input type="text" class="text" name="phone"/></td></tr>
				<tr><th> 惰性粉气溶胶名称：</th><td><input type="text" class="text" name="drug"/></td><th> 施粉设施及方法：</th><td><input type="text" class="text" name="deviceandway"/></td></tr>
				<tr><th> 惰性粉气溶胶使用规模（吨/年）：</th><td><input type="text" name="totaluse" class="text"/></td><th> 使用惰性粉气溶胶的粮食比例（%）：</th><td><input type="text" name="proportion" class="text"/></td></tr>
			</table>
		    <!-- 成本核算 -->
			<table class="input tabContent">
				<tr><th> 施粉器械（元）：</th><td><input type="text" name="device" class="text"/></td><th>空仓杀虫剂药剂（品牌）：</th><td><input type="text" name="brand" class="text"/></td> </tr>
				<tr><th> 惰性粉气溶胶用量（kg）：</th><td><input type="text" name="dosage" class="text"/></td><th> 惰性粉气溶胶费用（元）：</th><td><input type="text" name="drugfee" class="text"/></td></tr>
				<tr><th> 防尘和过滤口罩等防护用品（元）：</th><td><input type="text" name="protective" class="text"/></td><th> 粉剂使用补助（元）：</th><td><input type="text" name="subsidy" class="text"/></td></tr>
				<tr><th> 劳务费用（元）：</th><td><input type="text" name="laborfee" class="text"/></td><th> 其它（元）：</th><td><input type="text" name="otherfee" class="text"/></td></tr>
			</table>
			<!-- 防护杀虫 -->
			<table class="input tabContent">
				<tr><th> 施粉方式：</th> <td><input type="text" name="applymethod" class="text" /></td></tr>
				<tr><th> 粮面施粉(mg/kg)：</th> <td><input type="text" name="applyonsurface" class="text" /></td><th> 分层施粉(mg/kg)：</th> <td><input type="text" name="applyeverylayer" class="text" /></td></tr>
				<tr><th> 整仓施粉(mg/kg)：</th> <td><input type="text" name="applyonwhole" class="text" /></td><th> 粮面处理(g/m2)：</th> <td><input type="text" name="dealonsurface" class="text" /></td></tr>
				<tr><th> 其他处理方式：</th><td><input type="text" name="otherdeal" class="text"  /></td></tr>
				<tr><th> 施粉时平均粮温(℃)：</th><td><input type="text" name="avggraintemperature" class="text"/></td></tr>
				<tr><th> 施粉时最高粮温(℃)：</th><td><input type="text" name="highestgraintemperature" class="text"/></td><th> 施粉时最低粮温(℃)：</th><td><input type="text" name="lowestgraintemperature" class="text"/></td></tr>
			</table>
			
			<!-- 防治效果 -->
			<table class="input tabContent">
				<tr><th> 施粉前虫口密度：</th> <td><input type="text" name="densitybef" class="text" /></td><th> 主要虫种：</th> <td><input type="text" name="kindbef" class="text" /></td></tr>
				<tr><th> 施粉后虫口密度（头/m2）：</th> <td><input type="text" name="densityafter" class="text" /></td><th> 主要虫种：</th> <td><input type="text" name="kindbef" class="text" /></td></tr>
				<tr><th> 无虫期间隔（天）：</th><td><input type="text" name="noninsects" class="text" /></td></tr>
			</table>
			<!-- 品质变化 -->
			<table class="input tabContent">
				<tr><th> 脂肪酸值（防治害虫前）：</th><td><input type="text" name="fattyvaluebef" class="text"/></td><th> 脂肪酸值（防治害虫后）：</th><td><input type="text" name="fattyvalueafter" class="text"/></td></tr>
				<tr><th> 粉剂残留(mg/kg)：</th><td><input type="text" name="residues" class="text"/></td><th> 其他：</th><td><input type="text" name="elsecharacter" class="text"/></td></tr>
