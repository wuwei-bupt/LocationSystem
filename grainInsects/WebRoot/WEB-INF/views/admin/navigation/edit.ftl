...
<title>${message("admin.navigation.edit")} - Powered By Szy++</title>
			...
			<tr>
				...
				<td>
					<select id="systemUrl">
						...
						<option value="${base}/catalogIndex.jhtml"[#if navigation.url == base + "/catalogIndex.jhtml"] selected="selected"[/#if]>${message("admin.navigation.catalogIndex")}</option>
						...
						[/#list]
						[#list catalogIndexTree as catalog]
							<option value="${base}${catalog.path}"[#if base + catalog.path == navigation.url] selected="selected"[/#if]>
								[#if catalog.grade != 0]
									[#list 1..catalog.grade as i]
										&nbsp;&nbsp;
									[/#list]
								[/#if]
								${catalog.mc}
