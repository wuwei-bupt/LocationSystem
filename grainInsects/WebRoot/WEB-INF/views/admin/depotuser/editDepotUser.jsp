...
/* 	//储粮区 combobox
	$('#graindirect').combobox({    
 	...
 	$('#graindirect').combobox('setValue','${user.graindepot.area.gd.id}');	//combobox中的值并没有加载成功，所以不能直接使用select函数
 	//$('#graindepotid').combobox('reload',"getGraindepot?graindirectionid" + "${user.graindepot.area.gd.id}");
	$('#graindepotid').combobox('setValue','${user.graindepot.lkbm}');
	$('#graindepotid').combobox('setText','${user.graindepot.lkmc}'); */
		...
		<form id="inputForm"    method="post" enctype="multipart/form-data">
			...
			<table class="input tabContent">
				...
				<tr><th><span class="requiredField">*</span> 粮库：</th> <td>${user.graindepot.lkmc}
					<input type="hidden" name="graindepotid" value="${user.graindepot.lkbm}"></td></tr>
