...
<script type="text/javascript">
	...
	$(function () {	
		console.log(Highcharts.getOptions().colors.length);
		...
		function loadBins(id) {
			$.getJSON('grainBins?lkbm=' + id, function(result) {
				result.push({
					lcbm:'全部粮仓'
				});
				$('#bin').combobox({
				    ...
				    onSelect:function() {
				    	...
				    	if (id != '全部粮仓') {
				    		...
				    		setParam('lkbm', $('#depot').combobox('getValue'));
				    		disableInput([{
			    				element:'#device',
				    			disabled:true	
				    			}]);
	...
	function autoSearch() {
		var lkbmStr = window.location.hash;
		var lkbm = '';
		if (lkbmStr.match(/#lkbm=/g)) {
			 parent.$.messager.progress({
				title: '提示',
				text: '自动选择粮库中.....'
			 });
			 
			lkbm = lkbmStr.split('=').pop();
			setParam('lkbm', lkbm);
			$.get('directionAndArea?lkbm=' + lkbm, function(data) {
				var directionId = data.directionId;
				var areaId = data.areaId;
				$('#direction').combobox('select', directionId);
				setTimeout(function(){
					$('#area').combobox('select', areaId);
					setTimeout(function(){
						$('#depot').combobox('select', lkbm);
...
<body class="easyui-layout" data-options="fit:true,border:false">
	<div region="north" title="监测预报 > 数据监控 > 过滤条件" style="height:100px ;overflow:hidden;">
		<div class="input-group" style="margin:10px">
			...
			粮仓:<input id="bin"  value="" disabled='disabled'>
	...
	<div data-options="region:'center',border:false" title="监测预报 > 数据监控" style="">
