...
<head>
	...
	<script type="text/javascript">
		...
		$(function(){
			if ($("#s").val() == "false") $("#s").next().attr('checked','checked');
			else $("#s").next().next().attr('checked','checked');
		...
		<form id="inputForm"  method="post" enctype="multipart/form-data">
			...
			<table class="input">
				...
				<tr>
					...
					<td><input name="title" class="myinput" type="text" style="width:60em" value="${title }"/></td>
				</tr>
				<tr>
					<th>是否公开：</th>
					<td>
						<input type="hidden" id="s" value="${hasshare }"/>
						<input name="hasshare" type="radio" value="false" />不公开&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
						<input name="hasshare" type="radio" value="true" />公开
					</td>
				</tr>				
				<tr>
					<th>类型：</th>
					<td><select name="type" id="type-select" class="myinput" style="width:150px">
						<option value="${type }" selected="selected">${typename }</option>
					</select></td>
				</tr>
				<tr>
					<th>描述：</th>
					<td><textarea id="editor" name="describle" class="editor" style="width: 98%;">${describle }</textarea></td>
				</tr>
				<tr>
					<th>咨询专家:</th>
					<td>
						<select id="expert-select" class="myinput" name="expertuser" style="width:150px">
							<option value="${expertuser }" selected="selected">${expertname }</option>
						</select>
					</td>
				</tr>			
			</table>
