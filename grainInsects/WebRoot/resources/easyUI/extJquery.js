...
//以post方式，打开新窗体
/* 具体调用如下
	window.open("", 'sjqx', "height=480,width=1000,top=100,left=200,toolbar=no, location=no, directories=no, status=no, menubar=no,copyhistory=no,channelmode=yes");
	openBlank('getQxEntrance', {'startid': rows[0].startid, 'endid' : rows[0].endid, 'id':rows[0].id, 
		'inweight': rows[0].inweight, 'outweight': rows[0].outweight,
		'station':  rows[0].station, 'carno':  rows[0].carno},'sjqx' );
		*/
