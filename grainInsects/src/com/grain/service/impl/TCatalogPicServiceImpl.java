...
/*
 * 分类特征图片 t_catalog_pic
 */
@Service("tCatalogPicServiceImpl")
public class TCatalogPicServiceImpl 
	extends BaseServiceImpl<TCatalogPic,Long>
	implements TCatalogPicService,ServletContextAware{
	 ...
	 *            原文件类型
	 */
	private void addTask(final String sourcePath, final String largePath, final String mediumPath, final String thumbnailPath,  final String watermarkString, final File tempFile, final String contentType) {
		try {
					...
					for (StoragePlugin storagePlugin : storagePlugins) {
						if (storagePlugin.getIsEnabled()) {
							...
							try {
								...
								if(watermarkString!=null && watermarkString !=""){
									ImageUtils.addWatermark(largeTempFile, largeTempFile, watermarkString, setting.getWatermarkPosition(), setting.getWatermarkAlpha());
									ImageUtils.addWatermark(mediumTempFile, mediumTempFile, watermarkString, setting.getWatermarkPosition(), setting.getWatermarkAlpha());
								}else{
	...
	public void build(TCatalogPic productImage) {
			...
			try {
				...
				for (StoragePlugin storagePlugin : storagePlugins) {
					if (storagePlugin.getIsEnabled()) {
						...
						String watermarkstr = productImage.getFromwhere()==null?"":productImage.getFromwhere();					
						addTask(sourcePath, largePath, mediumPath, thumbnailPath, watermarkstr, tempFile, multipartFile.getContentType());
