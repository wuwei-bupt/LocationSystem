...
/**
 * @author szy 权限验证拦截器 库管员权限拦截器
 */

public class DepoterInterceptor implements HandlerInterceptor {
	private transient final Log log = LogFactory.getLog(this.getClass());
	// 根路径
	// 登陆表单路径
	private static String loginPath = "depoter/common/login";
	// 静态资源路径
	private static String resourcesPath = "resources";
	// 登陆提交路径
	private static String submitPath = "depoter/common/loginsubmit";
	// 注销
	private static String logoff = "depoter/common/logout";
	// 主页面
	private static String mainPath = "depoter/common/main";
	// 验证码
	private static String captchaPath = "depoter/common/captcha";
	...
	public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object handler) throws Exception {
		...
		if (session.getAttribute(CommonAttributes.Principal) == null) {
			...
			String role = depotuserSrv.userRole(username);
			if (role != null && (role.equalsIgnoreCase("depotmanager") || role.equalsIgnoreCase("depotuser"))) {
				return true;
			} else {
				session.invalidate();
				return true;
