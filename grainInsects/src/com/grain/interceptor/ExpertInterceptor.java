...
public class ExpertInterceptor implements HandlerInterceptor {
	...
	// 根路径
	// 登陆表单路径
	private static String loginPath = "expert/common/login";
	// 静态资源路径
	private static String resourcesPath = "resources";
	// 登陆提交路径
	private static String submitPath = "expert/common/loginsubmit";
	// 注销
	private static String logoff = "expert/common/logout";
	// 主页面
	private static String mainPath = "expert/common/main";
	// 验证码
	private static String captchaPath = "expert/common/captcha";
	...
	public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object handler) throws Exception {
		...
		if (session.getAttribute(CommonAttributes.Principal) == null) {
			...
			String role = expertSrv.userRole(username);
			if (role != null && role.equalsIgnoreCase("expert")) {
				return true;
			} else {
				session.invalidate();
				return true;
