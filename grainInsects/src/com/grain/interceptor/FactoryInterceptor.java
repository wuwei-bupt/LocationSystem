...
public class FactoryInterceptor implements HandlerInterceptor {
	...
	// 根路径
	// 登陆表单路径
	private static String loginPath = "factory/common/login";
	// 静态资源路径
	private static String resourcesPath = "resources";
	// 登陆提交路径
	private static String submitPath = "factory/common/loginsubmit";
	// 注销
	private static String logoff = "factory/common/logout";
	// 主页面
	private static String mainPath = "factory/common/main";
	// 验证码
	private static String captchaPath = "factory/common/captcha";
	
	@Resource(name = "factoryuserServiceImpl")
	FactoryuserService factoryuserSrv;
	...
	public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object handler) throws Exception {
		...
		if (session.getAttribute(CommonAttributes.Principal) == null) {
			...
			String role = factoryuserSrv.userRole(username);
			if (role != null && role.equalsIgnoreCase("factoryuser")) {
				return true;
			} else {
				session.invalidate();
				return true;
