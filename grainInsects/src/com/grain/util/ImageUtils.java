...
import com.grain.Setting.WatermarkPosition;
...
public final class ImageUtils {
	 ...
	 *            水印透明度
	 */
	public static void addWatermark(File srcFile, File destFile, String watermarkString, WatermarkPosition watermarkPosition, int alpha) {
		Assert.notNull(srcFile);
		Assert.notNull(destFile);
		Assert.state(alpha >= 0);
		Assert.state(alpha <= 100);
		if (watermarkString == null  || watermarkPosition == null || watermarkPosition == WatermarkPosition.no) {
			try {
				FileUtils.copyFile(srcFile, destFile);
			} catch (IOException e) {
				e.printStackTrace();
		...
		if (type == Type.jdk) {
			...
			try {
				...
				int watermarkImageWidth = fm.stringWidth(watermarkString);
				int watermarkImageHeight = fm.getHeight();
				...
				graphics2D.drawString(watermarkString, x, y);
			...
			operation.dissolve(alpha);
			operation.quality((double) DEST_QUALITY);
			operation.write(watermarkString);
			operation.addImage(srcFile.getPath());
			operation.addImage(destFile.getPath());
			if (type == Type.graphicsMagick) {
				CompositeCmd compositeCmd = new CompositeCmd(true);
